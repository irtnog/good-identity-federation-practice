...
DOCUMENTS=$(or $(shell git ls-tree --full-tree --name-only -r HEAD docs))
DRAFTS=$(or $(shell git ls-files --others --exclude-standard docs))
...
html: $(PYPACKAGE_NAME).egg-info $(DOCUMENTS) $(DRAFTS)
	$(SPHINXBUILD) -M $@ docs docs/_build $(SPHINXOPTS)
